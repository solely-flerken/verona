...
describe('restaurant — slot states', () => {
    it('open: inside a slot', () => {
        ...
        expect(result).toEqual({isOpen: true, label: 'Geöffnet · bis 22:00 Uhr'})
    })
    ...
    it('before: before first slot', () => {
        ...
        expect(result).toEqual({isOpen: false, label: 'Öffnet heute um 17:00 Uhr'})
    })

    it('between: after first slot, before second (split shift)', () => {
        const result = getOpeningStatus(standard, TUE(15, 30))
        expect(result).toEqual({isOpen: false, label: 'Öffnet wieder um 17:00 Uhr'})
    })

    it('open: inside first slot of split shift', () => {
        const result = getOpeningStatus(standard, TUE(12))
        expect(result).toEqual({isOpen: true, label: 'Geöffnet · bis 14:00 Uhr'})
    })
    ...
    it('afterClose: after last slot', () => {
        ...
        expect(result).toEqual({isOpen: false, label: 'Geschlossen'})
    })
    ...
    it('closed: rest day', () => {
        ...
        expect(result).toEqual({isOpen: false, label: 'Heute geschlossen'})
    })

    it('sunday opens earlier', () => {
        const result = getOpeningStatus(standard, SUN(15))
        expect(result).toEqual({isOpen: false, label: 'Öffnet heute um 16:00 Uhr'})
    })
...
describe('restaurant — overrides', () => {
    ...
    it('annual override: named holiday is closed', () => {
        ...
        expect(getOpeningStatus(withOverrides, christmas)).toEqual({isOpen: false, label: 'Heiligabend · geschlossen'})
    })

    it('annual override: applies every year', () => {
        const christmas2027 = at(2027, 12, 24, 15)
        expect(getOpeningStatus(withOverrides, christmas2027)).toEqual({isOpen: false, label: 'Heiligabend · geschlossen'})
    })

    it('annual override: unnamed falls back to closed label', () => {
        const nye = at(2026, 12, 31, 20)
        expect(getOpeningStatus(withOverrides, nye)).toEqual({isOpen: false, label: 'Heute geschlossen'})
    })

    it('one-time override: open with special hours', () => {
        const special = at(2026, 10, 3, 14)
        expect(getOpeningStatus(withOverrides, special)).toEqual({isOpen: true, label: 'Geöffnet · bis 18:00 Uhr'})
    })

    it('one-time override: outside special hours is afterClose', () => {
        const special = at(2026, 10, 3, 20)
        expect(getOpeningStatus(withOverrides, special)).toEqual({isOpen: false, label: 'Geschlossen'})
    })
    ...
    it('exact date override beats annual override on same DD.MM', () => {
        ...
        expect(result).toEqual({isOpen: true, label: 'Geöffnet · bis 20:00 Uhr'})
    })
})

describe('delivery mode', () => {
    it('open', () => {
        expect(getOpeningStatus(standard, WED(19), 'delivery')).toEqual({isOpen: true, label: 'Lieferservice · bis 22:00 Uhr'})
    })

    it('before', () => {
        expect(getOpeningStatus(standard, THU(15), 'delivery')).toEqual({isOpen: false, label: 'Lieferservice ab 17:00 Uhr'})
    })

    it('afterClose', () => {
        expect(getOpeningStatus(standard, FRI(23), 'delivery')).toEqual({isOpen: false, label: 'Lieferservice geschlossen'})
    })
    ...
    it('named holiday override', () => {
        ...
        expect(getOpeningStatus(config, at(2026, 12, 24, 15), 'delivery')).toEqual({isOpen: false, label: 'Heiligabend · kein Lieferservice'})
    })
})
